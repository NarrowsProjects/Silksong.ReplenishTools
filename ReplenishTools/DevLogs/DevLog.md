...
- Enemy Interaction
  - The class `EnemyHitEffectsRegular` has the public method called `ReceiveHitEffect`, which creates a new HitInstance instance (object initializer syntax) with a direction (which is a float??) and an attack type. In this case the attack type is generic, which is likely a normal strike with the needle.
